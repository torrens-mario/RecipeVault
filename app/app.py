...
from database import (
    init_db, list_recipes, get_recipe, create_recipe, update_recipe,
    delete_recipe, toggle_favorite, toggle_planned_to_cook,
    list_inventory, create_inventory_item, update_inventory_item,
    delete_inventory_item, consume_ingredients_for_recipe,
    list_low_stock_items, get_shopping_list
)
...
@app.get("/api/recipes")
def api_list_recipes(q: str | None = None):
    return [r.model_dump() for r in list_recipes(q)]
...
def api_create_recipe(payload: RecipeCreate):
    return create_recipe(payload).model_dump()
...
def api_cook_recipe(recipe_id: int):
    ...
    result["low_stock"] = list_low_stock_items()
    return result
...
def api_update_inventory_item(item_id: int, payload: dict):
    ...
    item = update_inventory_item(
        item_id=item_id, name=name, quantity=quantity, unit=unit,
        low_stock_threshold=threshold, low_stock_unit=threshold_unit
    )
    if not item:
