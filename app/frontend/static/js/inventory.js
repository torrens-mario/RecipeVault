...
function openEditModal(item) {
  ...
  if ([...thresholdSelect.options].some(o => o.value === thresholdValue)) {
    thresholdSelect.value = thresholdValue;
  } else {
    thresholdSelect.value = '5';
  }
  // FIX: correctly set low_stock_unit in the modal
...
async function renderLowStock() {
  ...
  if (!panel) {
    ...
    document.querySelector('.container').insertBefore(panel, document.querySelector('section'));
  }
  if (!list.length) { panel.innerHTML = ''; return; }
  panel.innerHTML = list.map(i => `
    <div class="low-stock-card">
      <strong>${i.name}</strong><br>
      <span class="muted">Quedan ${i.quantity} ${i.unit}. Conviene comprar antes de quedarte sin.</span>
    </div>`).join('');
}
      ...
      <td>
        <div class="inventory-name">
          ...
          <div>
            ...
            ${i.quantity <= (i.low_stock_threshold || 0) ? '<div class="low-stock-badge">Stock bajo</div>' : ''}
          </div>
        </div>
      </td>
      <td>${i.quantity}</td>
      <td>${i.unit}</td>
      <td>${i.low_stock_threshold ?? 5} ${i.low_stock_unit || i.unit}</td>
      <td style="text-align:right;">
        <button class="btn secondary" data-edit="${i.id}">Editar</button>
        <button class="btn secondary" data-del="${i.id}">Borrar</button>
      </td>
    </tr>`).join('');
  renderLowStock();
}
...
editForm?.addEventListener('submit', async (e) => {
  ...
  // FIX: include low_stock_unit in the update payload
  const payload = {
    name: fd.get('name'),
    quantity: Number(fd.get('quantity') || 0),
    unit: fd.get('unit'),
